...
namespace MnestixSearcher.Clients.Model
{
    ...
    public partial class Resource : IValidatableObject
    {
        ...
        [JsonConstructor]
        public Resource(Option<string?> path = default, Option<string?> contentType = default)
        {
            PathOption = path;
    ...
    public class ResourceJsonConverter : JsonConverter<Resource>
    {
        ...
        public override Resource Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (path.IsSet && path.Value == null)
                throw new ArgumentNullException(nameof(path), "Property is not nullable for class Resource.");

            if (contentType.IsSet && contentType.Value == null)
                throw new ArgumentNullException(nameof(contentType), "Property is not nullable for class Resource.");

            return new Resource(path, contentType);
        }
        ...
        {
            if (resource.PathOption.IsSet && resource.Path == null)
                throw new ArgumentNullException(nameof(resource.Path), "Property is required for class Resource.");

            if (resource.ContentTypeOption.IsSet && resource.ContentType == null)
                throw new ArgumentNullException(nameof(resource.ContentType), "Property is required for class Resource.");

            if (resource.PathOption.IsSet)
                writer.WriteString("path", resource.Path);

            if (resource.ContentTypeOption.IsSet)
                writer.WriteString("contentType", resource.ContentType);
        }
    }
}
